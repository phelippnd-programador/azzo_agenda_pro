...
import type { ClientImportErrorLine, ClientImportJob } from "@/types/client-import";
import { formatDateTime } from "@/pages/stock/utils";

const RUNNING_STATUSES = ["RECEBIDO", "EM_VALIDACAO", "PROCESSANDO"];

export default function ClientImportDetailPage() {
  const { jobId } = useParams();
  const navigate = useNavigate();
  const [job, setJob] = useState<ClientImportJob | null>(null);
  const [errors, setErrors] = useState<ClientImportErrorLine[]>([]);
  ...
  const loadJob = async () => {
    if (!jobId) return;
    const response = await clientImportApi.getImportJobById(jobId);
    setJob(response);
  };
  ...
  const loadErrors = async () => {
    ...
    try {
      const response = await clientImportApi.getImportErrors(jobId);
      setErrors(response);
    } catch (error) {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      navigate("/clientes/importacoes", { replace: true });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <PageEmptyState
        ...
        action={
          <Button asChild variant="outline">
            <Link to="/clientes/importacoes">Voltar para importacoes</Link>
          </Button>
        }
      />
    );
