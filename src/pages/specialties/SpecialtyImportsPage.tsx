...
const IMPORT_MODE_LABELS: Record<SpecialtyImportMode, string> = {
  INSERT_ONLY: "Inserir somente novas",
  UPSERT: "Inserir e atualizar existentes",
};

export default function SpecialtyImportsPage() {
  const [jobs, setJobs] = useState<SpecialtyImportJob[]>([]);
  const [errorsByJob, setErrorsByJob] = useState<Record<string, SpecialtyImportErrorLine[]>>({});
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [modoImportacao, setModoImportacao] = useState<SpecialtyImportMode>("INSERT_ONLY");
  const [formatoModelo, setFormatoModelo] = useState<SpecialtyImportTemplateFormat>("xlsx");
  ...
  const load = async () => {
    try {
      setIsLoading(true);
      const list = await specialtyImportApi.listImportJobs();
      setJobs(list);
    } catch (error) {
      toast.error(
        resolveUiError(error, "Nao foi possivel carregar importacoes de especialidades.").message
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const created = await specialtyImportApi.createImportJob({
        arquivo: selectedFile,
        modoImportacao,
        dryRun,
      });
      toast.success("Importacao de especialidades enviada com sucesso.");
      setJobs((prev) => [created, ...prev]);
      setSelectedFile(null);
      setPage(1);
    } catch (error) {
      toast.error(resolveUiError(error, "Erro ao enviar importacao de especialidades.").message);
    } finally {
  ...
  const handleLoadErrors = async (jobId: string) => {
    try {
      const errors = await specialtyImportApi.getImportErrors(jobId);
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const blob = await specialtyImportApi.downloadImportTemplate({ formato: formatoModelo });
      const fileName = `modelo-importacao-especialidades.${formatoModelo}`;
  ...
  return (
    <div className="space-y-4">
      <Card className="border-border/80">
        <CardHeader>
          <CardTitle>Nova importacao de especialidades</CardTitle>
          <p className="text-sm text-muted-foreground">
            Envie a planilha, escolha o modo de processamento e acompanhe os jobs mais recentes.
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Quando ativado, o sistema valida o arquivo e mostra erros, mas nao cria nem atualiza especialidades.
          </p>
      ...
      <Card className="border-border/80">
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {hasRunningJob ? <Badge variant="default">Atualizando automaticamente</Badge> : null}
          </div>
        </CardHeader>
        <CardContent className="space-y-2">
          {isLoading ? (
            ...
            <PageEmptyState
              ...
              description="Envie um arquivo para criar o primeiro job de importacao de especialidades."
            />
          ) : (
            pagedJobs.map((job) => {
              ...
              return (
                <div key={job.jobId} className="rounded-xl border border-border/70 bg-card p-4">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
                    ...
                    <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                      <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Sucesso</p>
                      <p className="mt-1 font-medium text-foreground">{job.linhasSucesso}</p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                      <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Erros</p>
                      <p className="mt-1 font-medium text-foreground">{job.linhasErro}</p>
                    </div>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <Button variant="outline" size="sm" asChild>
                      <Link to={`/especialidades/importacoes/${job.jobId}`}>Detalhes</Link>
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => void handleLoadErrors(job.jobId)}>
                      Ver erros
                    </Button>
                    <Button
                      variant="outline"
