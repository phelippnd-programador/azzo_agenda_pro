...
import type { ServiceImportErrorLine, ServiceImportJob } from "@/types/service-import";
import { formatDateTime } from "@/pages/stock/utils";

const RUNNING_STATUSES = ["RECEBIDO", "EM_VALIDACAO", "PROCESSANDO"];

export default function ServiceImportDetailPage() {
  const { jobId } = useParams();
  const navigate = useNavigate();
  const [job, setJob] = useState<ServiceImportJob | null>(null);
  const [errors, setErrors] = useState<ServiceImportErrorLine[]>([]);
  ...
  const loadJob = async () => {
    if (!jobId) return;
    const response = await serviceImportApi.getImportJobById(jobId);
    setJob(response);
  };
  ...
  const loadErrors = async () => {
    ...
    try {
      const response = await serviceImportApi.getImportErrors(jobId);
      setErrors(response);
    } catch (error) {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      navigate("/servicos/importacoes", { replace: true });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <PageEmptyState
        ...
        action={
          <Button asChild variant="outline">
            <Link to="/servicos/importacoes">Voltar para importacoes</Link>
          </Button>
        }
      />
    );
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="space-y-1">
            ...
            {job.mensagemResumo ? (
              <p className="text-xs text-muted-foreground">{job.mensagemResumo}</p>
            ) : null}
            {job.errorMessage ? (
              ...
              <div className="space-y-2">
                {pagedErrors.map((line, index) => (
                  ...
                  >
                    ...
                    <p className="text-muted-foreground"><strong>Valor recebido:</strong> {line.valorOriginal || "-"}</p>
                  </div>
                ))}
              </div>
              <PaginationControls
