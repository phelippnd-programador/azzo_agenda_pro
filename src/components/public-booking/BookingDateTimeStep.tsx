      ...
      <CardContent className="space-y-5 sm:space-y-6">
          ...
          <div className="grid grid-cols-7 gap-1">
            {getDaysInMonth().map((date, i) => (
              <button
                ...
                className={`aspect-square rounded-lg text-xs font-medium transition-colors sm:text-sm ${
                  !date
                    ? 'invisible'
        ...
        <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            {selectedTime ? (
              <div className="w-fit rounded-lg bg-primary/10 px-3 py-2 text-sm font-medium text-primary">
                {selectedTime}
              </div>
            ) : null}
          </div>
          ...
          {selectedDate ? (
            visibleSlots.length > 0 ? (
              ...
              <div className="mt-4 rounded-xl border border-dashed border-border/70 bg-background/80 px-4 py-6 text-center text-sm text-muted-foreground">
