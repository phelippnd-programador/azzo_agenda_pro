...
interface NfseInvoicesEmbedProps {
  /** Quando fornecido, exibe botao "Nova NFS-e" que aciona o callback */
  onNewNfse?: () => void;
}

export function NfseInvoicesEmbed({ onNewNfse }: NfseInvoicesEmbedProps) {
  const now = new Date();
  ...
  const filtered = items.filter((invoice) => {
    const key =
      `${invoice.numeroNfse || ""} ${invoice.numeroRps} ${invoice.customer?.name || ""} ${invoice.fiscalStatus}`.toLowerCase();
    ...
    <Card>
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          {onNewNfse ? (
            <Button onClick={onNewNfse}>Nova NFS-e</Button>
          ) : (
            <Button asChild>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td className="p-2 text-muted-foreground" colSpan={5}>
                    {isLoading ? "Carregando..." : "Nenhuma NFS-e encontrada."}
                  </td>
                </tr>
              ) : (
