...
| RF-EST-001..020 | Estoque | `/estoque`, `/estoque/itens`, `/estoque/movimentacoes`, `/estoque/importacoes` | `/estoque/itens*`, `/estoque/movimentacoes*`, `/estoque/importacoes*`, `/estoque/importacoes/modelo`, `/estoque/dashboard` | CRUD de item/movimentacao, importacao assincrona, download de modelo, healthcheck e metricas | commits backend `60961b3`, `ef6f037`, `d28f9d1`, `99f10a9`, `be60a14`, `6a31717` + `mvn -DskipTests compile` + `mvn -Dtest=CalculadoraEstoqueUnitTest,ImportacaoEstoquePolicyUnitTest test` | EM_EXECUCAO | Backend/Frontend | RBAC por `stock:view`/`stock:manage`; URL assinada com expiracao curta; storage key segregada por tenant (`tenant/{tenantId}/...`) |
